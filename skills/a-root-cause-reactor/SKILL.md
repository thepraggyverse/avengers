---
name: a-root-cause-reactor
description: Avengers skill A - Root Cause Reactor that helps the user to find the reactor-level cause that powers repeated failures. Use when the user mentions or needs one of these triggers - root cause, why keeps happening, reactor, underlying cause. Also use when searching for Avengers, Tony Stark, Iron Man, MCU, A-skill, or A-prefixed operating modes.
---
...
Tier 2 Problem Solving skill. Find the reactor-level cause that powers repeated failures.

## Use When

- The user needs to find the reactor-level cause that powers repeated failures.
- The request matches these triggers: root cause, why keeps happening, reactor, underlying cause.
...
```text
Use $a-root-cause-reactor on this messy situation and give me the next concrete move.
```
...
If the user asks for find the reactor-level cause that powers repeated failures., return the output contract fields and a concrete next action.
